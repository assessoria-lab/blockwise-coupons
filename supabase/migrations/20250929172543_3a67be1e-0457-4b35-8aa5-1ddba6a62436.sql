-- Dropar todas as versões da função criar_loja_apos_signup
DROP FUNCTION IF EXISTS public.criar_loja_apos_signup(text, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.criar_loja_apos_signup(uuid, text, text, text, text, text, text);

-- Recriar a função com todos os parâmetros corretos incluindo user_id
