-- =====================================================
-- MIGRAÇÃO: Estrutura de Dados Otimizada - Correção
-- =====================================================

-- 1. Melhorar tabela de lojistas (adicionar campos ausentes)
ALTER TABLE lojistas 
ADD COLUMN IF NOT EXISTS nome_responsavel TEXT,
ADD COLUMN IF NOT EXISTS ddd VARCHAR(3),
ADD COLUMN IF NOT EXISTS whatsapp VARCHAR(20),
ADD COLUMN IF NOT EXISTS estado VARCHAR(2) DEFAULT 'GO',
...
-- Atualizar constraints e índices para lojistas
CREATE UNIQUE INDEX IF NOT EXISTS idx_lojistas_whatsapp ON lojistas(whatsapp);
...
-- 2. Melhorar tabela de clientes (adicionar campos ausentes)
ALTER TABLE clientes 
ADD COLUMN IF NOT EXISTS data_primeiro_cupom TIMESTAMPTZ DEFAULT NOW(),
ADD COLUMN IF NOT EXISTS total_cupons_recebidos INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS total_valor_compras DECIMAL(12,2) DEFAULT 0.00,
...
CREATE INDEX IF NOT EXISTS idx_pagamentos_referencia ON pagamentos(referencia_externa);

-- RLS para pagamentos
ALTER TABLE pagamentos ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin pode gerenciar pagamentos"
ON pagamentos FOR ALL
USING (true);

-- 4. Criar tabela de transações WhatsApp
...
-- Índices para transações whatsapp
...
CREATE INDEX IF NOT EXISTS idx_transacoes_status ON transacoes_whatsapp(status_processamento);

-- RLS para transações whatsapp
ALTER TABLE transacoes_whatsapp ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admin pode gerenciar transações whatsapp"
ON transacoes_whatsapp FOR ALL
USING (true);

-- 5. Criar tabela de usuários administrativos
