-- Atualizar a função atribuir_cupons_para_cliente para incluir cidade do cliente
CREATE OR REPLACE FUNCTION public.atribuir_cupons_para_cliente(
    ...
    p_cliente_cidade character varying,
...
BEGIN
    ...
    -- 3. Busca ou cria cliente (agora com cidade)
    SELECT id INTO v_cliente_id FROM clientes WHERE cpf = p_cliente_cpf;

    IF v_cliente_id IS NULL THEN
        INSERT INTO clientes (nome, cpf, telefone, cidade)
        VALUES (p_cliente_nome, p_cliente_cpf, p_cliente_telefone, p_cliente_cidade)
        RETURNING id INTO v_cliente_id;
    ELSE
        -- Atualiza dados do cliente existente se necessário (incluindo cidade)
        UPDATE clientes
        SET
